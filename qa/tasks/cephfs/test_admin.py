...
class TestLabeledPerfCounters(CephFSTestCase):
    ...
    def test_per_client_labeled_perf_counters(self):
        ...
        fs_suffix = dump["mds_client_metrics"]["labels"]["fs_name"]
        self.assertGreaterEqual(dump["mds_client_metrics"]["counters"]["num_clients"], 0)
        ...
        counters_dump_a = [c["labels"]["counters"] for \
                           c in dump[per_client_metrics_key] if c["labels"]["client"] == mount_a_id][0]
        counters_dump_b = [c["labels"]["counters"] for \
        ...
        for i in range(100):
            self.fs.open_background(basename=f'test_dir/test_file_{i}', write=False)
        with safe_while(sleep=1, tries=30, action=f'wait for counters - {mount_a_id}') as proceed:
            while proceed():
                if counters_dump_a["total_read_ops"] > 0 and counters_dump_a["total_read_size"] > 0:
                    return True
