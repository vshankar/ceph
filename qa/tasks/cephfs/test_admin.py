...
class TestLabeledPerfCounters(CephFSTestCase):
    ...
    def test_per_client_labeled_perf_counters(self):
        ...
        clients = [c["labels"]["client"] for c in dump[per_client_metrics_key][0]]
        self.assertIn(mount_a_id, clients)
        self.assertIn(mount_b_id, clients)

        counters_dump_a = [c["labels"]["counters"] for \
                           c in dump[per_client_metrics_key][0] if c["labels"]["client"] == mount_a_id][0]
        counters_dump_b = [c["labels"]["counters"] for \
                           c in dump[per_client_metrics_key][0] if c["labels"]["client"] == mount_b_id][0]
