...
class TestHardlinkRemoteAuth(CephFSTestCase):
    ...
    def _setup_subtrees(self, path1, path2, path3):
        ...
        # pins
        self.mount_a.setfattr(path1, "ceph.dir.pin", "0")
        self.mount_a.setfattr(path2, "ceph.dir.pin", "1")
        self.mount_a.setfattr(path3, "ceph.dir.pin", "2")
    ...
    def test_hardlink_remote_auth(self):
        ...
        self._setup_subtrees(path1, path2, path3)
        ...
        st = self.mount_a.lstat(f'{path2}/yfile')
        self.mount_a.lstat(f'{path3}/zfile')

        idump = self.fs.rank_tell(['dump', 'inode', str(st['ino'])], rank=0)
        log.debug(f"inode: {st['inode']}")
        log.debug(f"path: {idump['path']}")
        log.debug(f"is_auth: {idump['is_auth']}")
        log.debug(f"auth_state: {idump['auth_state']}")
        log.debug(f"replica_state: {idump['replica_state']}")
