...
class TestMirroring(CephFSTestCase):
    ...
    def test_directory_command_ls(self):
        ...
        self.enable_mirroring(self.primary_fs_name, self.primary_fs_id)
        try:
            self.add_directory(self.primary_fs_name, self.primary_fs_id, dir1)
            self.add_directory(self.primary_fs_name, self.primary_fs_id, dir2)
            time.sleep(10)
            ...
            res = self.mirror_daemon_command(f'mirror status for fs: {self.primary_fs_name}',
                                             'fs', 'mirror', 'status', f'{self.primary_fs_name}@{self.primary_fs_id}')
            dir_count = res['snap_dirs']['dir_count']
            self.assertTrue(len(dirs_list) == dir_count and dir1 in dirs_list and dir2 in dirs_list)
        except CommandFailedError:
            raise RuntimeError('Error listing directories')
        except AssertionError:
            raise RuntimeError('Wrong number of directories listed')
        finally:
            self.remove_directory(self.primary_fs_name, self.primary_fs_id, dir1)
            self.remove_directory(self.primary_fs_name, self.primary_fs_id, dir2)
