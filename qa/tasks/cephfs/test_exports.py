...
class TestEphemeralRandomDirfrags(CephFSTestCase):
    ...
    def test_ephemeral_random_max_config(self):
        ...
        valid probability ranges and reject out-of-bounds values.
        """
        self.mount_a.run_shell(["mkdir", "-p", "test_config_dir"])

        # Attempt to set invalid random probability values (> 1.0 or < 0.0)
        for invalid_val in ["-0.1", "1.5", "foo"]:
            with self.assertRaises(CommandFailedError) as cm:
                self.mount_a.setfattr("test_config_dir", "ceph.dir.pin.random", invalid_val)
    ...
    def test_ephemeral_pin_shrink_mds(self):
        """
        Verify that reducing active MDS cluster size evacuates decommissioned
        ranks and preserves valid random dirfrag subtrees across remaining ranks.
        """
        factor = 8
        self.fs.set_max_mds(3)
        self.status = self.fs.wait_for_daemons()

        self._setup_split_dir(path="shrink_dir", random_prob=1.0, total_files=100, factor=factor)

        # Allow proactive splitting to reach steady state across 3 ranks (target: 2^5 = 32 frags)
        time.sleep(20)
        subtrees_old = self._wait_random_subtrees(
            16,
            status=self.status,
        ...
        self.assertGreaterEqual(len(old_frags), 16)
        ...
        # Retrieve new subtree layout post-shrink
        subtrees_new = self._get_subtrees(status=self.status, rank="all", path="/shrink_dir")
        new_frags = {
            s['dir']['dirfrag']: s['auth_first']
            for s in subtrees_new
            if s['dir']['path'] == '/shrink_dir'
        ...
        log.info(f"Fragments before shrink: {len(old_frags)}, after shrink: {len(new_frags)}")

        # Fragment count post-shrink should remain at or above the 2-rank floor (16 frags)
        self.assertGreaterEqual(len(new_frags), 16)

        # Decommissioned rank 2 must be fully evacuated
        auth_ranks = set(new_frags.values())
        self.assertTrue(all(auth in (0, 1) for auth in auth_ranks))

        # Both remaining ranks must hold fragments
        self.assertIn(0, auth_ranks)
        self.assertIn(1, auth_ranks)
