...
void Server::handle_client_file_blockdiff(const MDRequestRef& mdr)
{
   ...
   * open ref, but we need the snapped inode. Probably could use
   * MDCache::get_inode().
   */
  ...
  auto snap_id = (uint64_t)req->head.args.blockdiff.snap_other;

  mdcache->file_blockdiff(in1, in2, &(ctx->block_diff), ctx);
}
