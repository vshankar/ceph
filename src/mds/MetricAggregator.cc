...
int MetricAggregator::init() {
  ...
  m_perf_counters.reset(plb.create_perf_counters());
  m_cct->get_perfcounters_collection()->add(m_perf_counters.get());
...
void MetricAggregator::shutdown() {
  ...
  if (m_perf_counters) {
    m_cct->get_perfcounters_collection()->remove(m_perf_counters.get());
  }
}
...
void MetricAggregator::refresh_metrics_for_rank(const entity_inst_t &client,
  ...
  if (ins) {
    ...
    client_perf_counters[client].reset(plb.create_perf_counters());
    m_cct->get_perfcounters_collection()->add(client_perf_counters[client].get());
  }

  // update perf counters
  auto perf_counter_ptr = client_perf_counters[client].get();
...
void MetricAggregator::remove_metrics_for_rank(const entity_inst_t &client,
  ...
  m_cct->get_perfcounters_collection()->remove(client_perf_counters[client].get());
  client_perf_counters.erase(client);
