...
void MetricAggregator::ping_all_active_ranks(const std::set<mds_rank_t> &active_ranks) {
  dout(10) << ": pinging " << active.size() << " active mds(s)" << dendl;

  for (auto &rank : active_ranks) {
...
int MetricAggregator::init() {
  ...
  pinger = std::thread([this]() {
      ...
      while (!stopping) {
        std::set<mds_rank_t> active_ranks(active);
        // drop this lock as mds pinger would acquire mds_lock
        locker.unlock();
        ping_all_active_ranks(active_ranks);
