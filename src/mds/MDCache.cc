...
struct C_ListSnapsAggregator : public MDSIOContext {
  ...
  void finish(int r) override {
    mds->mdcache->aggreggate_snap_sets(snap_set_context, in1, in2,
...
void MDCache::file_blockdiff(CInode *in1, CInode *in2, BlockDiff *block_diff, MDSContext *ctx) {
  ...
  uint64_t scans = std::min(
    ...
    (uint64_t)(g_conf().get_val<uint64_t>("mds_file_bllockdiff_max_concurrent_object_scans")));
...
void MDCache::aggreggate_snap_sets(const std::vector<std::unique_ptr<SnapSetContext>> &snap_set_ctx,
