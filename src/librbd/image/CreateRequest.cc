...
CreateRequest<I>::CreateRequest(IoCtx &ioctx, std::string &imgname, std::string &imageid,
  ...
  m_imgname(imgname), m_imageid(imageid), m_size(size), m_order(order),
  ...
  m_id_obj = util::id_obj_name(m_imgname);
  m_header_obj = util::header_name(m_imageid);
  m_objmap_name = ObjectMap::object_map_name(m_imageid, CEPH_NOSNAP);
...
void CreateRequest<I>::create_id_object() {
  ...
  op.create(true);
  cls_client::set_id(&op, m_imageid);
...
void CreateRequest<I>::add_image_to_directory() {
  ...
  librados::ObjectWriteOperation op;
  cls_client::dir_add_image(&op, m_imgname, m_imageid);
...
void CreateRequest<I>::journal_create() {
  ...
  Journal<I>::init_safetimer_instance(m_cct, &m_timer, &m_timer_lock);

  // there's no need for calling ->init() as there aren't any journal
  // objects to remove in case of failure from now on in the state
  // machine -- header object purging is handled by ->remove()
  m_journaler = new Journaler(m_op_work_queue, m_timer, m_timer_lock,
                              m_ioctx, m_imageid,
                              librbd::Journal<I>::IMAGE_CLIENT_ID,
                              m_cct->_conf->rbd_journal_commit_age);
  ...
  librbd::journal::CreateJournal<I> *req = librbd::journal::CreateJournal<I>::create(
    m_ioctx, m_imageid, m_journal_order, m_journal_splay_width, m_journal_pool,
    cls::journal::Tag::TAG_CLASS_NEW, tag_data, m_journaler, ctx);
  req->send();
}
...
void CreateRequest<I>::fetch_mirror_image() {
  ...
  librados::ObjectReadOperation op;
  cls_client::mirror_image_get_start(&op, m_imageid);
...
void CreateRequest<I>::mirror_image_enable() {
  ...
  librados::ObjectWriteOperation op;
  cls_client::mirror_image_set(&op, m_imageid, m_mirror_image_internal);
...
void CreateRequest<I>::send_watcher_notification() {
  ...
  Context *ctx = new FunctionContext([this](int r) {
      r = MirroringWatcher<>::notify_image_updated(
        ...
        m_imageid, m_mirror_image_internal.global_image_id);
...
void CreateRequest<I>::complete(int r) {
  ...
  using klass = CreateRequest<I>;
  Context *ctx = create_context_callback<klass, &klass::finish>(this);

  shut_down_and_cleanup_journaler(ctx);
}

template<typename I>
Context *CreateRequest<I>::finish(int *result) {
  ldout(m_cct, 20) << __func__ << ": r=" << *result << dendl;
...
// helper
template<typename I>
void CreateRequest<I>::shut_down_and_cleanup_journaler(Context *ctx) {
  ldout(m_cct, 20) << this << " " << __func__ << dendl;
...
void CreateRequest<I>::journal_remove() {
  ...
  librbd::journal::RemoveJournal<I> *req = librbd::journal::RemoveJournal<I>::create(
    m_ioctx, m_imageid, m_journaler, ctx);
  req->send();
}
...
template<typename I>
void CreateRequest<I>::shut_down_journaler() {
  ldout(m_cct, 20) << this << " " << __func__ << dendl;

  using klass = CreateRequest<I>;
  Context *ctx = create_context_callback<klass, &klass::handle_shut_down_journaler>(this);

  shut_down_and_cleanup_journaler(ctx);
}

template<typename I>
Context *CreateRequest<I>::handle_shut_down_journaler(int *result) {
  ldout(m_cct, 20) << __func__ << ": r=" << *result << dendl;
...
void CreateRequest<I>::remove_from_dir() {
  ...
  librados::ObjectWriteOperation op;
  cls_client::dir_remove_image(&op, m_imgname, m_imageid);
