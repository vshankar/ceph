...
int Journal<I>::create(librados::IoCtx &io_ctx, const std::string &image_id,
  ...
  journal::TagData tag_data;
  tag_data.mirror_uuid = (non_primary ? primary_mirror_uuid
                          : LOCAL_MIRROR_UUID);

  C_SaferCond cond;
  journal::CreateJournal<I> *req = journal::CreateJournal<I>::create(
    io_ctx, image_id, order, splay_width, object_pool,
    cls::journal::Tag::TAG_CLASS_NEW, tag_data, &journaler, &cond);
  req->send();
