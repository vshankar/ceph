...
class FSPolicy(object):
    ...
    def add_directory(self, dir_path):
        with self.lock:
            ...
            schedule = self.policy.add_directory(dir_path)
            if not schedule:
                return
    ...
    def remove_directory(self, dir_path):
        with self.lock:
            ...
            schedule = self.policy.remove_directory(dir_path)
            if schedule:
                self.schedule_action([dir_path])
...
class FSSnapshotMirror(object):
    ...
    def verify_and_set_mirror_info(self, local_fs_name, remote_cluster_spec, remote_fs_name):
        ...
        if local_cluster_id == remote_cluster_id and local_fs_name == remote_fs_name:
            raise MirrorException(-errno.EINVAL, 'matching local and remote cluster spec')
    ...
    def verify_and_normalize_dir(self, fs_handle, dir_path):
        norm_path = os.path.normpath(dir_path)
        try:
            stx = fs_handle.statx(norm_path, cephfs.CEPH_STATX_MODE, cephfs.AT_SYMLINK_NOFOLLOW)
            if not stat.S_ISDIR(stx["mode"]):
                raise MirrorException(-errno.EINVAL, f'path {dir_path} should be a directory')
        except cephfs.ObjectNotFound:
            raise MirrorException(-errno.ENOENT, f'{dir_path} does not exist')
        except cephfs.Error as e:
            log.error(f'failed to verify directory {dir_path}: {e}')
            raise Exception(-e.errno)
        return norm_path

    def verify_and_normalize_path(self, filesystem, dir_path):
        if not os.path.isabs(dir_path):
            raise MirrorException(-errno.EINVAL, f'{dir_path} should be an absolute path')
        try:
            with open_filesystem(self.local_fs, filesystem) as local_fs_handle:
                return self.verify_and_normalize_dir(local_fs_handle, dir_path)
        except CephfsConnectionException as e:
            raise MirrorException(-e.errno, e.error_str)

    def add_directory(self, filesystem, dir_path):
        try:
            with self.lock:
                ...
                if not fspolicy:
                    raise MirrorException(-errno.EINVAL, f'filesystem {filesystem} is not mirrored')
                dir_path = self.verify_and_normalize_path(filesystem, dir_path)
                log.debug(f'path normalized to {dir_path}')
                fspolicy.add_directory(dir_path)
                return 0, json.dumps({}), ''
        except MirrorException as me:
            return me.args[0], '', me.args[1]
        except Exception as e:
    ...
    def remove_directory(self, filesystem, dir_path):
        try:
            with self.lock:
                if not self.filesystem_exist(filesystem):
                    raise MirrorException(-errno.ENOENT, f'filesystem {filesystem} does not exist')
                ...
                if not fspolicy:
                    raise MirrorException(-errno.EINVAL, f'filesystem {filesystem} is not mirrored')
                dir_path = self.verify_and_normalize_path(filesystem, dir_path)
                fspolicy.remove_directory(dir_path)
                return 0, json.dumps({}), ''
        except MirrorException as me:
            return me.args[0], '', me.args[1]
        except Exception as e:
