...
class FSSnapshotMirror:
    ...
    @staticmethod
    def peer_config_key(filesystem, key):
        return f'{FSSnapshotMirror.PEER_CONFIG_KEY_PREFIX}/{filesystem}/{key}'
    ...
    def purge_mirror_info(self, local_fs_name, peer_uuid):
        ...
        # try to fetch auth details from config store
        cmd = {'prefix': 'config-key get',
               'key': FSSnapshotMirror.peer_config_key(local_fs_name, peer_uuid)}
        r, outs, err = self.mgr.mon_command(cmd)
        if r < 0 and not r == -errno.ENOENT:
            log.error(f'mon command to get config-key failed: {err}')
            raise Exception(-errno.EINVAL)
        remote_conf = {}
        if r == 0:
            remote_conf = json.loads(outs)
    ...
    def peer_add(self, filesystem, remote_cluster_spec, remote_fs_name, remote_conf):
        try:
            ...
            with self.lock:
                ...
                if remote_conf['mon_host'] and remote_conf['key']:
                    cmd = {'prefix': 'config-key set',
                           'key': FSSnapshotMirror.peer_config_key(filesystem, peer_uuid),
                           'val': json.dumps(remote_conf)}
                    r, outs, err = self.mgr.mon_command(cmd)
                    if r < 0:
                        log.error(f'mon command to set config-key failed: {err}')
                        raise Exception(-errno.EINVAL)
    ...
    def peer_remove(self, filesystem, peer_uuid):
        try:
            with self.lock:
                ...
                r, outs, err = self.mgr.mon_command(cmd)
                if r < 0 and not r == -errno.ENOENT:
                    log.error(f'mon command to remove config-key failed: {err}')
                    raise Exception(-errno.EINVAL)
                return 0, json.dumps({}), ''
        except MirrorException as me:
            return me.args[0], '', me.args[1]
