...
std::map<std::string, std::string> decode_snap_metadata(const char *metadata_dict) {
  std::map<std::string, std::string> metadata;
  while (*metadata_dict != '\0') {
    const char* key = metadata_dict;
    metadata_dict += strlen(key) + 1;
    const char* value = metadata_dict;
    metadata_dict += strlen(value) + 1;
...
int encode_snap_metadata(std::map<std::string, std::string> metadata,
                         char **metadata_dict) {
  uint64_t len = 0;
  ...
  if (len) {
    ...
    len = 0;
    for (auto &[key, value] : metadata) {
      memcpy(ptr + len, key.c_str(), key.size());
      len += key.size();
      ptr[len] = '\0';
      ++len;
      memcpy(ptr + len, value.c_str(), value.size());
      len += value.size();
      ptr[len] = '\0';
      ++len;
    }
  ...
  for (auto &snap : snaps) {
    ...
    if (is_remote) {
      if (!info.metadata_len) {
        ...
        auto metadata = decode_snap_metadata(info.metadata);
...
int PeerReplayer::synchronize(const std::string &dir_path, uint64_t snap_id,
  ...
  std::map<std::string,std::string> metadata{{PRIMARY_SNAP_ID_KEY, stringify(snap_id)}};
  char *metadata_dict;
  r = encode_snap_metadata(metadata, &metadata_dict);
  if (r < 0) {
    derr << ": error encoding metadata: " << cpp_strerror(r) << dendl;
    return r;
  }
