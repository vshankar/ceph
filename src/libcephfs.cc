...
extern "C" int ceph_fchmodat(struct ceph_mount_info *cmount, int dirfd, const char *relpath,
                             mode_t mode, int flags) {
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fchmodat(dirfd, relpath, mode, flags, cmount->default_perms);
}
...
extern "C" int ceph_fchownat(struct ceph_mount_info *cmount, int dirfd, const char *relpath,
                             uid_t uid, gid_t gid, int flags) {
  if (!cmount->is_mounted())
    return -ENOTCONN;
  return cmount->get_client()->fchownat(dirfd, relpath, uid, gid, flags, cmount->default_perms);
}
